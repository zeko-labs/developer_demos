...
const zkappKeyEnv = getSecret('ZKAPP_PRIVATE_KEY');

const deployerSource = process.env.DEPLOYER_PRIVATE_KEY ? 'env' : 'missing';
const zkappSource = process.env.ZKAPP_PRIVATE_KEY ? 'env' : 'missing';

if (!deployerKey || !zkappKeyEnv) {
  throw new Error('Missing DEPLOYER_PRIVATE_KEY or ZKAPP_PRIVATE_KEY in .env');
}
