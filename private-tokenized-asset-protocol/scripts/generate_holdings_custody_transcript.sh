...
OUT_FILE="${OUT_DIR}/holdings-custody-demo-${STAMP}.md"
...
{
  echo "# TAP Holdings Custody Demo Transcript"
  echo
  ...
  echo
  echo "- Custody holdings collection settles through the source adapter path."
  echo "- Custody webhook signature verification succeeds."
  echo "- Settlement artifacts show policy-linked holdings evidence."
  echo
} >"${OUT_FILE}"
...
capture "Bootstrap Custody Provider" "${ROOT_DIR}/scripts/bootstrap_holdings_custody.sh"
capture "Run Holdings Custody Demo" "${ROOT_DIR}/scripts/run_holdings_custody_demo.sh"
...
if [[ "${REDACT_PUBLIC_COPY}" == "1" ]]; then
  REDACTED_FILE="${OUT_DIR}/holdings-custody-demo-${STAMP}.public.md"
  sed -E \
