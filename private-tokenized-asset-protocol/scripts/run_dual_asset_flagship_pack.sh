...
API_LOG="${API_LOG:-/tmp/tap-api-dual-asset-flagship.log}"
...
echo "[2/7] Starting isolated API (${API_BASE_URL})..."
PORT="${PORT}" \
...
echo "[4/7] Running dual-asset flagship transcript..."
...
LATEST_TRANSCRIPT="$(find "${ROOT_DIR}/output/demo-transcripts" -maxdepth 1 -type f -name 'dual-asset-flagship-demo-*.md' | grep -v '\.public\.md$' | sort | tail -n 1)"
LATEST_PUBLIC="$(find "${ROOT_DIR}/output/demo-transcripts" -maxdepth 1 -type f -name 'dual-asset-flagship-demo-*.public.md' | sort | tail -n 1)"

echo "[5/7] Verifying flagship transcript..."
./scripts/verify_transcript.sh "${LATEST_TRANSCRIPT}"

echo "[6/7] Packaging public artifact..."
PUBLIC_DIR="${ROOT_DIR}/output/demo-transcripts/public-pack"
mkdir -p "${PUBLIC_DIR}"
cp "${LATEST_PUBLIC}" "${PUBLIC_DIR}/"
cp "${LATEST_PUBLIC}" "${PUBLIC_DIR}/flagship-dual-asset-latest.public.md"
...
echo "flagship transcript: ${LATEST_TRANSCRIPT}"
echo "flagship public transcript: ${LATEST_PUBLIC}"
