...
SUBJECT_COMMITMENT="${SUBJECT_COMMITMENT:-subj_zktls_demo_001}"
RUN_PIPELINE_FIRST="${RUN_PIPELINE_FIRST:-0}"
MODE="${MODE:-eligible}"
OUT_DIR="${OUT_DIR:-${ROOT_DIR}/output/demo-transcripts}"
STAMP="$(date -u +"%Y-%m-%dT%H-%M-%SZ")"
OUT_FILE="${OUT_DIR}/zktls-source-collect-demo-${STAMP}.md"
...
{
  echo "# TAP zkTLS Source Collect Demo Transcript"
  echo
  ...
  echo
  echo "- zkTLS-backed source collection succeeds through the normal source/collect path."
  echo "- Eligibility proof is generated and settled with policy snapshot linkage."
  echo "- Recent settlements include provider=zktls-employer metadata."
  echo
} >"${OUT_FILE}"
...
capture "Run zkTLS Source Collect Demo" "API_BASE_URL='${API_BASE_URL}' TENANT_API_KEY='${TENANT_API_KEY}' ADMIN_API_KEY='${ADMIN_API_KEY}' TENANT_ID='${TENANT_ID}' POLICY_ID='${POLICY_ID}' SUBJECT_COMMITMENT='${SUBJECT_COMMITMENT}' RUN_PIPELINE_FIRST='${RUN_PIPELINE_FIRST}' MODE='${MODE}' '${ROOT_DIR}/scripts/run_zktls_source_collect_demo.sh'"
