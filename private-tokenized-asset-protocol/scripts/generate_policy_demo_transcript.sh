...
OUT_FILE="${OUT_DIR}/policy-linkage-demo-${STAMP}.md"
...
{
  echo "# TAP Policy Linkage Demo Transcript"
  echo
  ...
  echo
  echo "- Settlement succeeds when proof linkage matches active policy."
  echo "- Settlement is rejected for stale or missing policy linkage."
  echo
} >"${OUT_FILE}"
...
capture "Seed Tenant Policy/Provider" "${ROOT_DIR}/scripts/bootstrap_policy_seed.sh"
capture "Run Policy Linkage Demo Flow" "${ROOT_DIR}/scripts/run_policy_linkage_demo.sh"
