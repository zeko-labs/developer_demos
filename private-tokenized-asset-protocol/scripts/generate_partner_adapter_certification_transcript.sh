...
OUT_FILE="${OUT_DIR}/partner-adapter-certification-demo-${STAMP}.md"
...
{
  echo "# TAP Partner Adapter Certification Transcript"
  echo
  ...
  echo
  echo "- Generic-rest adapter invalid config returns expected error."
  echo "- Domain allowlist violation returns expected error."
  echo "- Failover path succeeds and returns settlement."
  echo
} >"${OUT_FILE}"
...
capture "Run Partner Adapter Certification" "${ROOT_DIR}/scripts/run_partner_adapter_certification.sh"
...
if [[ "${REDACT_PUBLIC_COPY}" == "1" ]]; then
  REDACTED_FILE="${OUT_DIR}/partner-adapter-certification-demo-${STAMP}.public.md"
  sed -E \
