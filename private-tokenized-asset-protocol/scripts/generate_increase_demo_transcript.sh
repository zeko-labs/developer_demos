...
OUT_FILE="${OUT_DIR}/increase-balance-demo-${STAMP}.md"
...
{
  echo "# TAP Increase Balance Demo Transcript"
  echo
  ...
  echo
  echo "- Increase balance source collection succeeds."
  echo "- Eligibility proof is generated and settled with policy snapshot linkage."
  echo "- Recent settlements include provider=increase metadata."
  echo
} >"${OUT_FILE}"
...
capture "Bootstrap Policy" "API_BASE_URL='${API_BASE_URL}' ADMIN_API_KEY='${ADMIN_API_KEY}' '${ROOT_DIR}/scripts/bootstrap_policy_seed.sh'"

if [[ -z "${INCREASE_ACCOUNT_ID}" ]]; then
  capture "Increase Demo Run (missing account)" "echo 'INCREASE_ACCOUNT_ID is required'; exit 1"
else
  capture "Run Increase Balance Demo" "API_BASE_URL='${API_BASE_URL}' TENANT_API_KEY='${TENANT_API_KEY}' ADMIN_API_KEY='${ADMIN_API_KEY}' TENANT_ID='${TENANT_ID}' POLICY_ID='${POLICY_ID}' SUBJECT_COMMITMENT='${SUBJECT_COMMITMENT}' INCREASE_ACCOUNT_ID='${INCREASE_ACCOUNT_ID}' BOOTSTRAP_POLICY=0 '${ROOT_DIR}/scripts/run_increase_balance_demo.sh'"
fi
