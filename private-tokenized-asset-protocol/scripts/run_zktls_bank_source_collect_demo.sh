...
SUBJECT_COMMITMENT="${SUBJECT_COMMITMENT:-subj_zktls_bank_collect_001}"
...
curl -sf "${API_BASE_URL}/api/v1/health"
echo

echo "[3/4] Source collect via zktls-bank..."
...
curl -sf -X POST "${API_BASE_URL}/api/v1/attest/source/collect" \
...
curl -sf "${API_BASE_URL}/api/v1/settlement/recent"
echo
